...
/**
 * Peticion GET a la API para obtener las nacionalidades
 ...
 */
  ...
  getRecipesByNatonality(nationality:string):Observable<{strMeal:string,strMealThumb:string,idMeal:string}[]>{
  ...
  getRecipesById(id:string):Observable<Meal | undefined>{
    return this.http.get(`${environment.api.viewRecipe}${id}`).pipe(map((res:any)=>{
