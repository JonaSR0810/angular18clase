...
export class HomeComponent {
  ...
  set type(type: string) {
    ...
    request.subscribe(
      (data: any) => {
        this.state.update(state => ({ ...state, loading: false, error: false, data: data.map((m :any)=> (type == "category" ? ({name: m.strCategory}) : ({name : m.strArea}))) }));
        console.log(data);
      },
      (err) => {
        console.log(err);
        this.state.update(state => ({ ...state, loading: false, error: true, data: [] }));
      }
    );
  }
